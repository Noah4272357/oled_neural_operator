...
Variant: P2 探针（2026-08-24）尺寸自适应副本（沉淀自 /tmp/analyze_p2.py）。

仓库规范脚本的样本索引上限硬编码为 5000/500/200（`range(0, 5000, 25)` 等），
对探针小数据集（neural_operator_3_pre，500/50/50）会越界崩溃。本变体按
manifest 实际 splits 大小推导索引（cap 与采样步长），对原尺寸数据集数值等价
（2026-08-24 P2 验证：基线数值与 2026-08-23/fourier_analysis.json 逐位一致）。

用法：
    python experiments/analysis/2026-08-24/p2_analyze_fourier.py \
        --data-root ~/data/neural_operator_3_pre \
        --output-dir experiments/analysis/2026-08-24
"""
...
def per_mode_ls_floor(
    ...
    """
    size_train = size_train or 5000
    size_test = size_test or 200
    rng_train = list(range(0, size_train, max(1, size_train // n_train)))[:n_train]
    rng_test = list(range(0, size_test, max(1, size_test // n_test)))[:n_test]
...
def main() -> None:
    ...
    for split, count in (("train", 20), ("val", 20), ("test", 20)):
        idxs = [min(i, sizes[split] - 1) for i in range(0, count * 7, 7)]
        rls = []
        for i in idxs:
    ...
    for split in ("train", "test"):
        idxs = [min(i, sizes[split] - 1) for i in range(0, 14 * 7, 7)]
        rls = []
        for i in idxs:
    ...
    ls_floors = {
        "raw_8ch_s_u": per_mode_ls_floor("train", "test", root, ("s", "u"), size_train=sizes["train"], size_test=sizes["test"]),
        "diff8ch_sddot_u": per_mode_ls_floor("train", "test", root, ("sddot", "u"), size_train=sizes["train"], size_test=sizes["test"]),
        "diff8ch_sddot_u_trim2": per_mode_ls_floor(
            "train", "test", root, ("sddot", "u"), trim=2, size_train=sizes["train"], size_test=sizes["test"]
        ),
        "diff8ch_sddot_u_trim4": per_mode_ls_floor(
            "train", "test", root, ("sddot", "u"), trim=4, size_train=sizes["train"], size_test=sizes["test"]
        ),
        "13ch": per_mode_ls_floor("train", "test", root, ("q", "qdot", "qddot", "u"), size_train=sizes["train"], size_test=sizes["test"]),
    }
    if args.reg != 1e-10:
        ls_floors = {
            f"{key}_reg{args.reg:g}": per_mode_ls_floor(
                "train", "test", root, fields, reg=args.reg,
                size_train=sizes["train"], size_test=sizes["test"],
            )
            for key, fields in (
                ("diff8ch", ("sddot", "u")),
                ("13ch", ("q", "qdot", "qddot", "u")),
