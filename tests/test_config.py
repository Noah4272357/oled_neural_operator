...
class ConfigTests(unittest.TestCase):
    ...
    def test_config_is_a_complete_recipe(self) -> None:
    ...
    def test_explicit_run_dir_is_used_verbatim(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            target = Path(directory) / "test_fixed"
            config = load_config(TEST_CONFIG)
    ...
    def test_generated_run_directories_do_not_collide(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            config = load_config(
                TEST_CONFIG, {"experiment": {"root": directory}}
            )
            first = resolve_run_dir(config)
            second = resolve_run_dir(config)
            self.assertTrue(first.name.startswith("test_"))
            self.assertNotEqual(first, second)

    def test_resolved_config_round_trips(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "config.yaml"
            config = load_config(TEST_CONFIG, {"training": {"epochs": 3}})
